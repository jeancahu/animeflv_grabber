...
LIST=$( bash ./get_animeflv_cap_list_from_tmpfile.sh $1 $TMP_FILE )

echo 'Iniciando descarga de vídeos'
# Se descarga cada uno de los capítulos disponibles:
for PAGE in $LIST
do
        echo "Descargando de: http://animeflv.net""$PAGE"
	bash ./download_zippyshare_from_animeflv_page.sh "http://animeflv.net""$PAGE"	
done
